...
class EditarClasePage extends StatefulWidget {
  final Map clase;

  const EditarClasePage({super.key, required this.clase});

  @override
  State<EditarClasePage> createState() => _EditarClasePageState();
}

class _EditarClasePageState extends State<EditarClasePage> {
  ...
  void initState() {
    super.initState();
    // Initialize controllers and selected values from widget.clase
    _nombreClaseController.text = widget.clase['nombre'] ?? '';
    _diaSemanaSeleccionado = widget.clase['dia_semana'];
    _horaInicio = widget.clase['hora_inicio'] ?? '';
    _horaFin = widget.clase['hora_fin'] ?? '';
    _lugaresDisponiblesController.text = widget.clase['lugares_disponibles']?.toString() ?? '';
    _idProfesorSeleccionado = widget.clase['id_profesor']; // Ensure this matches your API response key for professor ID
    _cargarProfesores();
  }
  ...
  Future<void> _seleccionarHora(bool esInicio) async {
    // Parse current time for initial selection
    TimeOfDay initialTime;
    try {
      final String timeString = esInicio ? _horaInicio : _horaFin;
        ...
        return Theme(
          data: ThemeData.light().copyWith(
            colorScheme: const ColorScheme.light(
              primary: Colors.indigo, // Color of buttons and selected items
              onPrimary: Colors.white, // Text color on primary color
              surface: Colors.white, // Background color of the dialog
              onSurface: Colors.blueGrey, // General text color
            ),
            textButtonTheme: TextButtonThemeData(
              style: TextButton.styleFrom(
                foregroundColor: Colors.indigo, // Color of "CANCEL" and "OK" buttons
              ),
            ),
          ),
          child: child!,
  ...
  Future<void> _actualizarClase() async {
    ...
    try {
      final response = await http.put(
        Uri.parse('http://127.0.0.1:8001/api/clases/${widget.clase['id']}'),
      ...
      if (response.statusCode == 200) {
        ...
        String successMessage = responseData['message'] ?? 'Clase actualizada con éxito.';
        ...
        String errorMessage = errorData['message'] ?? errorData['error'] ?? 'Hubo un error al actualizar la clase.';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Editar Clase', style: TextStyle(color: Colors.white)),
                    ...
                    children: [
                      Expanded(
                        child: _buildTimePickerField(
                          ...
                          time: _horaInicio.isNotEmpty ? _horaInicio.substring(0, 5) : '', // Show HH:MM
                      ...
                      Expanded(
                        child: _buildTimePickerField(
                          label: 'Hora de Fin',
                          time: _horaFin.isNotEmpty ? _horaFin.substring(0, 5) : '', // Show HH:MM
